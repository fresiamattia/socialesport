...
interface Course {
  ...
  acf?: {
    sottotitolo?: string;
    brochure_multisport?: string; // Campo ACF per l'URL della brochure MULTISPORT
    tipologia_corso_multisport?: string; // Campo ACF per l'etichetta Multisport
...
export default async function MultisportPage() {
  ...
  const pageTitle = "MULTISPORT"; // Titolo specifico come richiesto

  // Filtra i corsi per la tassonomia 'multisport'
  const multisportCourses = allCourses.filter(course => {
    const tipologiaCorsoTerms = course._embedded?.['wp:term']?.[0];
    return tipologiaCorsoTerms?.some(term => term.taxonomy === 'tipologia_corso' && term.slug === 'multisport');
  });

  // Trova il primo corso di multisport per recuperare il campo ACF brochure_multisport
  const firstMultisportCourse = multisportCourses.length > 0 ? multisportCourses[0] : null;
  const brochureUrl = firstMultisportCourse?.acf?.brochure_multisport || '#'; // Usa '#' come fallback

  // Definisci i colori per Multisport
  const titleColorClass = 'text-green-700'; // Verde scuro per multisport
  const labelBgColorClass = 'bg-green-700'; // Verde scuro per l'etichetta
  ...
  return (
    <div className="container mx-auto p-4 md:p-8 font-sans">
      ...
      {multisportCourses.length === 0 ? (
        <p className="text-center text-gray-600">Nessun corso Multisport trovato al momento.</p>
      ) : (
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-8">
          {multisportCourses.map(course => {
            ...
            const labelText = course.acf?.tipologia_corso_multisport; // Etichetta specifica per Multisport
              ...
              <Link key={course.id} href={`/corsi/${course.slug}`} className="block group">
                ...
                <div className="bg-white rounded-lg shadow-md overflow-hidden transform transition-transform duration-300 hover:scale-105 hover:shadow-xl flex flex-col h-full">
                  ...
                  <div className="p-6 flex flex-col flex-grow">
                    ...
                    <button className="inline-flex items-center mt-auto px-4 py-2 bg-green-600 text-white text-sm font-semibold rounded-md hover:bg-green-700 transition-colors duration-300 self-start">
